...
func (uq *UserQuery) QueryComments() *CommentsQuery {
	query := &CommentsQuery{config: uq.config}
...
func (uq *UserQuery) QueryPosts() *ExperienceQuery {
	query := &ExperienceQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithComments(opts ...func(*CommentsQuery)) *UserQuery {
	query := &CommentsQuery{config: uq.config}
...
func (uq *UserQuery) WithPosts(opts ...func(*ExperienceQuery)) *UserQuery {
	query := &ExperienceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withPosts; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Posts = []*Experience{}
		}
		query.withFKs = true
